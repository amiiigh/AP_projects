...
// {
// 	vector<string> userslist = user.get_users_list();
// 	for (int i =0 ; i< userslist.size(); i++)
// 	{
// 		comboulist->addItem(QString::fromStdString(userslist[i]));
// 	}
// }

// void UI::seeprof_clicked()
// {
	// user.see_user(comboulist->currentText().toStdString);
	// if (ewg!=NULL)
	// 	ewg->hide();
	// User *user;
	// if (insta.admin_log)
...
{	
	...
	try
	{
		...
		user.login(line_user->text().toStdString(),line_pass->text().toStdString());
	}
	catch(Ex ex)
	{
...
void UI::signup_clicked()
{
	...
	try
	{
		if (line_user->text().toStdString() == "" || line_pass->text().toStdString() == "")
			throw Ex("fill the empty fields");
		user.register_user(line_user->text().toStdString(),line_pass->text().toStdString());
	}
	catch(Ex ex)
	{
		QMessageBox::critical(this,"error occured!",QString::fromStdString(ex.get_message()));
